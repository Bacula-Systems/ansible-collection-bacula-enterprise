...
1) To deploy Bacula Enterprise Edition (DIR, SD, and FD) `12.6.3` on a host with the FQDN `baculadir.example.com` using the --extra_vars command line option, and the `bee.yml` playbook:

> \# ansible-playbook -i baculadir.example.com, tests/bee.yml --extra-vars "director_hostname=baculadir.example.com bee_version=12.6.3"
